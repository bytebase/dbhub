---
title: "Custom Tools"
description: "Define reusable, parameterized SQL operations in your TOML configuration"
---
...
The `readonly` field is only valid on `execute_sql`; configuring it on a custom tool is a startup validation error. Custom tools always execute their statement — DBHub still analyzes the statement to classify it as read or write, but that classification only sets the tool's `readOnlyHint` annotation (a hint for MCP clients), it does not block writes.
...
# Custom tools cannot set `readonly` and are unaffected by execute_sql's setting
[[tools]]
name = "get_user_by_id"
source = "prod_pg"
...
<Warning>
If you need a custom tool restricted to reads, keep its `statement` read-only yourself — DBHub does not enforce this for custom tools.
</Warning>
