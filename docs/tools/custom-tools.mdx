...
`readonly` is configured per tool. A custom tool with `readonly = true` uses
the same keyword pre-check, connector-side enforcement, and stable error
mapping as `execute_sql`. In particular, a custom tool backed by standard
MySQL Community Server 5.7, 8.x, and 9.x uses the same target-connection `sql_mode`
check and dangerous-function guardrail; it does not have a separate or weaker
SQL classifier. The fixed runtime validation matrix covers MySQL 5.7.44,
8.0.36, 8.4.10, and 9.7.1; other minor versions are not claimed as identically
...
# Configure readonly independently for this custom tool
[[tools]]
name = "get_user_by_id"
source = "prod_pg"
...
For the exact standard-MySQL function list, supported `sql_mode` values, and
flavor boundary, see [Execute SQL: Read-Only Mode](/tools/execute-sql#read-only-mode).
The application guardrail remains defense in depth; use a least-privilege,
