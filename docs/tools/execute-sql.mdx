...
| `source_id` | string | ❌ | first source | Database source identifier for multi-database setups |
| `transaction` | boolean | ❌ | `false` | Enable transaction mode for atomic execution |
| `max_rows` | integer | ❌ | `--max-rows` setting | Override the row limit for this query |
