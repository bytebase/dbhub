...
description: "Search and explore database schemas, tables, columns, indexes, and procedures with progressive disclosure"
---
...
- `object_type` (required): Type of object to search - `"schema"`, `"table"`, `"view"`, `"column"`, `"procedure"`, `"function"`, or `"index"`
...
- `table` (optional): Filter to a specific table (requires `schema`; only applies when `object_type` is `"column"` or `"index"`)
