...
<ParamField path="--readonly" type="boolean" env="READONLY" default="false">
  Restrict SQL execution to read-only operations. Only [allowed SQL keywords](https://github.com/bytebase/dbhub/blob/main/src/utils/allowed-keywords.ts) are permitted.

  Recommended for production databases to prevent accidental modifications.
...
<ParamField path="--max-rows" type="number" env="MAX_ROWS">
  ...
  - If your query has a `LIMIT` or `TOP` clause, DBHub uses the smaller value
