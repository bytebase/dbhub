...
<ParamField path="connection_timeout" type="number">
  ...
  Supported databases: PostgreSQL, MySQL, MariaDB, SQL Server, Redis (not applicable to SQLite).
...
<ParamField path="mode" type="string">
  Redis deployment mode. Defaults to `single`.

  Supported values: `single`, `cluster`, `sentinel`.
  ...
  type = "redis"
  mode = "cluster"
  nodes = ["redis-node-1.example.com:7000", "redis-node-2.example.com:7001"]
  user = "default"
  password = "secret"
  sslmode = "require"
  ...
  Cluster mode only supports Redis database `0`.
</ParamField>
...
| `mode` | string | ❌ | Redis only: `single`, `cluster`, or `sentinel` |
| `nodes` | array | ❌ | Redis Cluster root nodes |
| `sentinels` | array | ❌ | Redis Sentinel root nodes |
| `sentinel_master` | string | ❌ | Redis Sentinel master name |
| `sentinel_user` | string | ❌ | Redis Sentinel ACL username |
| `sentinel_password` | string | ❌ | Redis Sentinel password |
