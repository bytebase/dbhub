...
<ParamField path="pool_max_connections" type="integer" default="10">
  Maximum number of connections in this PostgreSQL source's pool. The value must be between 1 and 1000.
  ...
  When omitted, the PostgreSQL driver default of 10 is unchanged. This limit applies per DBHub process and per source, so include every running process and source when budgeting database connections.

  <Note>
  This option is PostgreSQL-only and is not supported as a DSN query parameter.
  </Note>
</ParamField>
