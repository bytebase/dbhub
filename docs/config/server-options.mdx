...
<ParamField path="--request-timeout" type="number" env="REQUEST_TIMEOUT">
  Request/query timeout in seconds. The maximum time to wait for a query to complete.
  ...
  ```bash
  npx @bytebase/dbhub@latest --dsn "sqlserver://..." --request-timeout 120
  ```
</ParamField>
...
| `--request-timeout` | `REQUEST_TIMEOUT` | number | - | Query timeout in seconds (SQL Server only) |
