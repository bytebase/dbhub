...
<ParamField path="sslmode" type="string">
  SSL/TLS mode for database connections. Available when using individual connection parameters (not DSN).
  ...
  **Options:**
  ...
  <Note>
  This option is only available via [TOML configuration](/config/multi-database) when using individual connection parameters. When using DSN, include `?sslmode=disable` or `?sslmode=require` in the connection string.
  </Note>
...
<ParamField path="instanceName" type="string">
  ...
  **SQL Server only.**
  ...
  <Note>
  This option is only available via [TOML configuration](/config/multi-database) when using individual connection parameters. When using DSN, include `?instanceName=INSTANCE` in the connection string.
  </Note>
...
<ParamField path="authentication" type="string">
  ...
  **Options:**
  - `ntlm` - Windows/NTLM authentication. Requires `domain` field and `password`.
  ...
  **SQL Server only.**
  ...
  - **DSN query parameter**: `?authentication=ntlm&domain=CORP` or `?authentication=azure-active-directory-access-token`
  - **TOML field**: `authentication = "ntlm"` (when using individual connection parameters)
...
<ParamField path="domain" type="string">
  ...
  **Required when `authentication = "ntlm"`. SQL Server only.**

  Can be configured via:
  - **DSN query parameter**: `?authentication=ntlm&domain=CORP`
  - **TOML field**: `domain = "CORP"` (when using individual connection parameters)
...
| - | - | number | - | Connection timeout in seconds (TOML only: `connection_timeout`) |
| - | - | number | - | Query timeout in seconds (TOML only: `query_timeout`) |
| - | - | string | - | SSL mode: disable, require (TOML only: `sslmode`) |
| - | - | string | - | SQL Server named instance (TOML only: `instanceName`) |
| - | - | string | - | SQL Server auth: ntlm, azure-active-directory-access-token (`authentication`) |
| - | - | string | - | Windows domain for NTLM (`domain`) |
