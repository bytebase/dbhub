...
<ParamField path="authentication" type="string">
  ...
  **SQL Server only.**
  ...
  <Note>
  This option is only available via [TOML configuration](/config/multi-database) when using individual connection parameters. When using DSN, include `?authentication=ntlm&domain=MYDOMAIN` in the connection string.
  </Note>
...
| - | - | string | - | SQL Server auth: ntlm, azure-active-directory-access-token (TOML only: `authentication`) |
| - | - | string | - | Windows domain for NTLM (TOML only: `domain`) |
