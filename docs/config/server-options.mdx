...
<ParamField path="--connection-timeout" type="number" env="CONNECTION_TIMEOUT">
  ...
  ```bash
  npx @bytebase/dbhub@latest --dsn "postgres://..." --connection-timeout 60
  ```
</ParamField>
...
<ParamField path="query_timeout" type="number">
  ...
  **Behavior by database:**
  ...
  |----------|---------------|
  | PostgreSQL | Client-side timeout via `query_timeout` option |
  | MySQL | Per-query timeout via `timeout` option |
  | MariaDB | Pool-level `queryTimeout` (requires MariaDB ≥10.1.2) |
  | SQL Server | Request timeout via `requestTimeout` option |
  | SQLite | Not supported (silently ignored) |
</ParamField>
...
| `--connection-timeout` | `CONNECTION_TIMEOUT` | number | - | Connection timeout in seconds |
