...
| `--ssh-host-key-check` | `SSH_HOST_KEY_CHECK` | string | Host key verification mode: `strict` (default), `accept-new`, or `off`. Defends against man-in-the-middle on the tunnel |
| `--ssh-known-hosts` | `SSH_KNOWN_HOSTS` | string | known_hosts file path(s) used for verification (default: `~/.ssh/known_hosts`) |
| `--ssh-host-fingerprint` | `SSH_HOST_FINGERPRINT` | string | Pin the SSH server host key by `SHA256:...` fingerprint instead of using known_hosts |
...
- When `--ssh-host` is a plain alias (no dots, not an IP address), DBHub automatically resolves it from `~/.ssh/config`, reading the `HostName`, `User`, `IdentityFile`, `ProxyJump`, `StrictHostKeyChecking`, and `UserKnownHostsFile` directives. Explicit flags always override values from the config file.
- Host key verification defaults to `strict`, which rejects a bastion whose key is not already trusted (in `known_hosts` or via a pinned fingerprint). This defeats a man-in-the-middle that impersonates the bastion. If a connection is rejected with a host key error on first use, add the host with `ssh-keyscan -H host >> ~/.ssh/known_hosts`, pin it with `--ssh-host-fingerprint`, or use `--ssh-host-key-check accept-new` to trust it on first use. `--ssh-host-key-check off` disables verification entirely and is insecure.
...
| `--ssh-host-key-check` | `SSH_HOST_KEY_CHECK` | string | Host key verification: `strict` (default), `accept-new`, `off` |
| `--ssh-known-hosts` | `SSH_KNOWN_HOSTS` | string | known_hosts file path(s) |
| `--ssh-host-fingerprint` | `SSH_HOST_FINGERPRINT` | string | Pinned `SHA256:...` host key fingerprint |