...
For Claude Desktop, the [MCP Bundle](/mcpb) is a one-click install: download the `.mcpb` from the [latest release](https://github.com/bytebase/dbhub/releases/latest), double-click, enter your connection string. Read-only by default, no Node.js or JSON editing required — the recommended path for non-technical users. The guide also covers packaging your own bundle.
