---
title: "Installation"
description: "Install DBHub via npm, Docker, MCP Bundle, or Claude Code Plugin — and configure it for Claude Desktop, Claude Code, Cursor, VS Code, Codex, Dify, and LibreChat"
---
...
Once DBHub is installed and running, configure your MCP client to connect to it. For HTTP transport configuration, see the [HTTP Server](/config/http-server) and [Authentication](/config/auth-token) guides. For SSH tunneling through bastion hosts, see [SSH Tunnel](/config/ssh-tunnel).
